...
const WaitMsg =
  'Your video is being processed, Please wait until the new video is generated';
...
function formatTime(seconds) {
  ...
  if (
    (parseFloat(seconds) === seconds || parseFloat(seconds) === 0) &&
    String(seconds).indexOf('.') === -1
  ) {
    time += '.000';
  }
  if (time.indexOf('.') === -1) {
...
class home extends Component {
  ...
  onLogin() {
    PopupTools.popup(
      `${API_URL}/video-cut-tool-back-end/login`,
      'Wiki Connect',
      ...
      axios
        ...
        .then(response => {
          ...
          if (Object.keys(pages)[0] !== '-1') {
            const { user, canonicaltitle, comment, url } = pages[
              Object.keys(pages)[0]
            ].videoinfo[0];
            ...
            resultObj.videos = [
              {
                author: user,
                title: decodeURIComponent(canonicaltitle.slice(5)).replace(
                  /\s/g,
                  '_',
                ),
                comment,
      ...
      resultObj.videos = [
        {
          ...
          title:
            this.state.uploadedFile === null
              ? decodeURIComponent(splittedUrl[splittedUrl.length - 1]).replace(
                  /\s/g,
                  '_',
                )
              : this.state.uploadedFile.name.replace(/\s/g, '_'),
          comment: '',
          text: '',
        },
      ];
    ...
    this.setState(
      ...
      () => {
        ...
        function getTruePropertyValue(property) {
          return parseFloat(
            getComputedStyle(resizerBlock, null)
        ...
        function getTransformValue() {
          return resizerBlock.style.transform
            .replace(/[^0-9\-.,]/g, '')
            .split(',')
            .map(Number);
        }
            ...
            switch (resizerPosition) {
              case 'top-center':
                ...
                if (height > minSize && transformValue[1] + top >= 0) {
                  resizerBlock.style.transform = `translate(${
                    transformValue[0]
                  }px, ${transformValue[1] + top}px)`;
              ...
              case 'bottom-center':
                ...
                if (
                  height > minSize &&
                  transformValue[1] + height <=
                    parseFloat(getPlayerCoords().height)
                ) {
                  resizerBlock.style.height = `${height}px`;
                }
                break;
              case 'left-center':
                ...
                if (width > minSize && transformValue[0] + left >= 0) {
                  resizerBlock.style.transform = `translate(${transformValue[0] +
                    left}px, ${transformValue[1]}px)`;
              ...
              case 'right-center':
                ...
                if (
                  width > minSize &&
                  transformValue[0] + width <=
                    parseFloat(getPlayerCoords().width)
                ) {
                  resizerBlock.style.width = `${width}px`;
                }
                break;
              case 'top-left':
                ...
                if (
                  width > minSize &&
                  transformValue[0] + (e.pageX - original_mouse_x) >= 0
                ) {
                  resizerBlock.style.width = `${width}px`;
                  left = e.pageX - original_mouse_x;
                }
                if (
                  height > minSize &&
                  transformValue[1] + (e.pageY - original_mouse_y) >= 0
                ) {
                  resizerBlock.style.height = `${height}px`;
                  top = e.pageY - original_mouse_y;
                }
                resizerBlock.style.transform = `translate(${transformValue[0] +
                  left}px, ${transformValue[1] + top}px)`;
                break;
              case 'top-right':
                width = original_width + (e.pageX - original_mouse_x);
                height = original_height - (e.pageY - original_mouse_y);
  ...
  validateVideoURL(url) {
    if (url !== '') {
      if (
        url.match(
          /^(?:https?:\/\/)?[\w.-]+(?:\.[\w.-]+)+[\w\-._~:/?#[\]@!$%&'()*+,;=.]+(?:mp4|webm|mov|flv|ogv)+$/g,
        ) !== null
      ) {
  ...
  onDragStop() {
    ...
    this.setState({
      duration: this.refs.player.video.video.duration,
    });
    ...
    if (!this.state.upload) {
      if (res.data.videos.length > 0) {
        res.data.videos.map((item, index) => {
          ...
          if (index > 0)
            newVideoTitle[0] = newVideoTitle[0].concat(`(${index})`);
...
{{self|cc-by-sa-4.0}}`,
  ...
  onSubmit(e) {
    ...
    if (this.state.uploadedFile !== null) {
      ...
      axios
        .post(`${API_URL}/video-cut-tool-back-end/send/upload`, data, {
          headers: {
      ...
      axios
        .post(`${API_URL}/video-cut-tool-back-end/send`, obj)
        .then(res => {
          this.previewCallback(res);
        })
        .catch(err => {
  ...
  checkVideoTitles() {
    ...
    return (
      !videoList.map(video => video.title.length > 0).includes(false) &&
  ...
  render() {
    ...
    return (
      <Layout className="layout">
        <Header>
          <Typography.Title level={1} onClick={() => window.location.reload()}>
            VideoCutTool
          </Typography.Title>
          <Menu theme="dark" mode="horizontal">
            {this.state.user ? (
              <>
                ...
                <Button primary className="c-auth-buttons__signout">
                ...
                <div style={{ padding: '1rem' }}>
                  <div className="docs-example" style={{ height: '100%' }}>
                          ...
                          onClick={() => {
                            if (this.state.uploadedFile !== null) {
                              this.updatePlayerInfo(
                                URL.createObjectURL(this.state.uploadedFile),
                              );
                              this.setState({
                                validateVideoURL: true,
                              });
                            } else if (
                              this.validateVideoURL(this.state.inputVideoUrl)
                            ) {
                    ...
                    {this.state.displayLoadingMessage ? (
                      ...
                      >
                        <p style={{ marginBottom: 0 }}>
                          Your video is {this.state.currentTask}...
                        </p>
                        <Progress
                          percent={this.state.progressPercentage}
                          status="active"
                          style={{ marginBottom: 10, padding: '0 5%' }}
                        />
                      </div>
                    ) : null}
                        ...
                        {this.state.videos.length > 1 ? (
                          <>
                            <p style={{ margin: '5px 0' }}>
                              Your New video
                              {this.state.videos.length > 1 && 's'} will be
                              here:
                            </p>
                            <ul>
                              {this.state.videos.map(video => (
                                <li>
                                  <a
                                    href={`https://commons.wikimedia.org/wiki/File:${video.title}`}
                                    target="_blank"
                          ...
                          <>
                            <p style={{ margin: '5px 0' }}>
                              Your New video will be here:{' '}
                              <a
                                href={`https://commons.wikimedia.org/wiki/File:${this.state.videos[0].title}`}
                                target="_blank"
                                rel="noopener noreferrer"
                              >
                                https://commons.wikimedia.org/wiki/File:
                    ...
                    {this.state.displayRotate ? (
                      <div>
                        <div
                          id={'RotatePlayerValue' + this.state.RotateValue}
                          style={{ marginTop: '8em' }}
                        >
                          <Player ref="player" videoId="video-1">
                            <BigPlayButton position="center" />
                            ...
                            <React.Fragment>
                              <Slider
                                ...
                                onChange={obj => {
                                  ...
                                  this.unsubscribeToStateChanges = this.refs.player.subscribeToStateChange(
                                    state => {
                  ...
                  <>
                    ...
                    {this.state.videos.map((video, index, videoArr) => (
                      <>
                              ...
                              <Col xl={12} xs={24}>
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    newVideoList[index] = {
                                      ...newVideoList[index],
                                      displayUploadToCommons: !newVideoList[
                                        index
                                      ].displayUploadToCommons,
                                    };
                            ...
                            {videoArr[index].displayUploadToCommons ? (
                              <>
                                ...
                                <div className="displayUploadToCommons">
                                  ...
                                  {videoArr[index].selectedOptionName ===
                                    'new-file' && (
                                    <>
                        ...
                        {this.state.videos.length > 1 &&
                          index < this.state.videos.length - 1 && <Divider />}
                      </>
                    ))}
                              ...
                              this.setState(
                                {
                                  upload: true,
                                  displayLoadingMessage: true,
                                  displaynewVideoName: true,
                                  loading: true,
                                  currentTask: 'uploading to Wikimedia Commons',
                                  progressPercentage: 0,
                                },
                                () => {
                                  this.onSubmit(e);
